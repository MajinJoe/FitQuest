  ...
  try {
    ...
    return searchData.foods
      .filter((food: any) => 
        ...
        food.fdcId // Ensure we have a valid ID
      )
      .slice(0, 15)
      .map((food: any) => {
        const nutrients = extractUSDANutrients(food.foodNutrients);
        
        const cleanedName = cleanUSDAFoodName(food.description);
        
        // Skip if name cleaning resulted in empty string
        ...
        return {
          id: `usda_${food.fdcId}`,
          name: cleanedName,
          brand: food.brandOwner?.trim() || food.marketClass?.trim() || null,
          category: mapUSDACategory(food.foodCategory || food.description),
          servingSize: "100g", // USDA data is per 100g
          calories: Math.max(0, Math.round(nutrients.calories || 0)),
          protein: Math.max(0, Math.round(nutrients.protein || 0)),
          carbs: Math.max(0, Math.round(nutrients.carbs || 0)),
          fat: Math.max(0, Math.round(nutrients.fat || 0)),
          fiber: Math.max(0, Math.round(nutrients.fiber || 0)),
          sugar: Math.max(0, Math.round(nutrients.sugar || 0)),
          sodium: Math.max(0, Math.round(nutrients.sodium || 0)),
          verified: true,
          barcode: null,
          source: "usda",
  ...
  try {
    ...
    return data.products
      .filter((p: any) => 
        ...
        p.code // Ensure we have a valid barcode/ID
      )
      .slice(0, 10)
      .map((p: any) => {
        ...
        return {
          ...
          calories: Math.max(0, Math.round(p.nutriments['energy-kcal_100g'] || p.nutriments['energy-kcal'] || 0)),
          protein: Math.max(0, Math.round(p.nutriments.proteins_100g || p.nutriments.proteins || 0)),
          carbs: Math.max(0, Math.round(p.nutriments.carbohydrates_100g || p.nutriments.carbohydrates || 0)),
          fat: Math.max(0, Math.round(p.nutriments.fat_100g || p.nutriments.fat || 0)),
          fiber: Math.max(0, Math.round(p.nutriments.fiber_100g || p.nutriments.fiber || 0)),
          sugar: Math.max(0, Math.round(p.nutriments.sugars_100g || p.nutriments.sugars || 0)),
          sodium: Math.max(0, Math.round((p.nutriments.sodium_100g || p.nutriments.sodium || 0) * 1000)), // Convert g to mg
          verified: true,
          barcode: p.code,
